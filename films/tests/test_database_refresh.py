...
class UpdatePeopleTestCase(unittest.TestCase):
    films_qs = MockSet()
    people_qs = MockSet()
    ...
    def setUp(self):
        ...
        self.films_qs.add(
            MockModel(
                id="2baf70d1-42bb-4437-b551-e5fed5a87abe",
                title="Castle in the Sky",
                descriptions="The orphan Sheeta inherited a mysterious crystal that links her to the mythical sky-kingdom of Laputa. With the help of resourceful Pazu and a rollicking band of sky pirates, she makes her way to the ruins of the once-great civilization. Sheeta and Pazu must outwit the evil Muska, who plans to use Laputa's science to make himself ruler of the world.",
                release_date="1986"
            )
        )

    def _mock_response(self, json_data=people_qs_mock_data, status=200):
        mock_resp = mock.Mock()
    ...
    def test_update_people(self, mock_get, people_qs=MockSet(), films_qs=MockSet()):
        mock_resp = self._mock_response(people_qs_mock_data)
    ...
    def test_update_films(self, mock_get, films_qs=MockSet()):
        mock_resp = self._mock_response(films_qs_mock_data)
        mock_get.return_value = mock_resp
        films_qs = self.films_qs
        update_films()